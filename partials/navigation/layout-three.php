...
?>
	...
	<header id="masthead" class="site-header layout-three <?php echo $header_style; ?>">
		<nav class="navbar navbar-expand-xl <?php echo $navbar_style; ?> <?php echo $scroll_state; ?>">
		  <div class="<?php echo $width; ?>">
		    ...
		    <div class="collapse navbar-collapse" id="navigationDesktop">
		      <ul class="navbar-nav me-auto align-items-center text-center">
		        <?php bootstrap_navigation(); ?>
		      </ul>
		    </div>

		    <a class="navbar-brand mx-auto mx-xl-0" href="<?php echo get_site_url(); ?>">
					<?php
	          $custom_logo_id = get_theme_mod( 'custom_logo' );
	          $logo = wp_get_attachment_image_src( $custom_logo_id , 'full' );
	          if ( has_custom_logo() ) {
	            echo '<img src="' . esc_url( $logo[0] ) . '" alt="' . get_bloginfo( 'name' ) . '" class="d-none d-xl-block" style="width: ' . $logo_width_desktop . 'px" />';
	            echo '<img src="' . esc_url( $logo[0] ) . '" alt="' . get_bloginfo( 'name' ) . '" class="d-block d-xl-none" style="width: ' . $logo_width_mobile . 'px" />';
	          } else {
			...
			<?php if( $call_to_action = get_field('header_call_to_action', 'options') ) { ?>
				<ul class="navbar-nav header-buttons order-xl-2 d-none d-xl-flex gap-2 ms-2 justify-content-end">
					<li class="nav-item">
			...
			<?php if( $phone_number ) { ?>
				<ul class="navbar-nav header-buttons order-xl-2 d-xl-none gap-2 ms-2 justify-content-end">
					<li class="nav-item">
